...
`[External Event (e.g. Telegram)]` $\rightarrow$ `[MCP Layer]` $\rightarrow$ `[OpenClaw Core]` $\rightarrow$ `[Internal Hook Trigger]` $\rightarrow$ **`[Shadow Logger Plugin]`** $\rightarrow$ `[Supabase (shadow_log.messages)]`

### Schema Design

The plugin writes to a dedicated, isolated schema to ensure security and separation from the public API: the `shadow_log.messages` table. This table is designed to capture the raw, unadulterated context of every interaction.

**Key Fields:**
- `id`: Unique identifier for the message entry.
...
The data recorded by this plugin serves as the raw material for **Layer 2: Semantic Distillation**. A separate background process (the "Refiner") will periodically consume the `shadow_log.messages` table to extract:
